...
static UniValue sendblock(const JSONRPCRequest& request)
{
    ...
    if (gArgs.IsArgSet("-kafka"))
    {
        ...
        int ret = post(gArgs.GetArg("-kafkaproxyhost", "localhost"), gArgs.GetArg("-kafkaproxyport", "8082"), "/topics/" + gArgs.GetArg("-kafkatopic", "ltc_test"), "{\"records\":[{\"value\":" + result.write() + "}]}", response_data);
        if (ret != 0)
        {
...
static UniValue sendbatchblock(const JSONRPCRequest& request)
{
	...
	for(int i = startHeight; i <= endHeight; i++)
	{
        ...
        {
            std::string response_data;
            int ret = post(gArgs.GetArg("-kafkaproxyhost", "localhost"), gArgs.GetArg("-kafkaproxyport", "8082"), "/topics/" + gArgs.GetArg("-kafkatopic", "ltc_test"), "{\"records\":[{\"value\":" + r.write() + "}]}", response_data);
            if (ret != 0)
            {
                std::cout << "error_code:" << ret << std::endl;
                std::cout << "error_message:" << response_data << std::endl;
