...
void myPrintBlockOrderByHeight(int& kafkaHeightRange)
{
	if (kafkaHeightRange <= chainActive.Height())
	{
		for (int i = kafkaHeightRange; i <= chainActive.Height(); i++)
		{
            ...
            if (i==1)
            {
                int ret = post(gArgs.GetArg("-kafkaproxyhost", "localhost"), gArgs.GetArg("-kafkaproxyport", "8082"), "/topics/" + gArgs.GetArg("-kafkatopic", "ltc_test"), "{\"records\":[{\"value\":" + myGetBlock(0).write() + "}]}", response_data);
                if (ret != 0)
                {
            ...
            int ret = post(gArgs.GetArg("-kafkaproxyhost", "localhost"), gArgs.GetArg("-kafkaproxyport", "8082"), "/topics/" + gArgs.GetArg("-kafkatopic", "ltc_test"), "{\"records\":[{\"value\":" + myGetBlock(i).write() + "}]}", response_data);
            if (ret != 0)
            {
                std::cout << "error_code:" << ret << std::endl;
                std::cout << "error_message:" << response_data << std::endl;
